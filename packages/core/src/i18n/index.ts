...
const zh: Dict = {
  ...
  'cli.noModelHint': '还没有配置模型 —— 输入 /connect provider 添加供应商后即可开始对话。',
  ...
  // ---- 连接向导（/connect） ----
  'cmd.connect.desc': '交互式添加模型供应商',
  'connect.usage': '用法：/connect provider',
  'connect.title': '连接向导',
  'connect.providerTitle': '添加模型供应商',
  'connect.step': '步骤 ${i}/${n}',
  'connect.chooseType': '选择协议类型',
  'connect.label': '供应商名称（展示用，如 DeepSeek）',
  'connect.apiKey': 'API Key',
  'connect.baseURL': 'Base URL（可留空用协议默认值；OpenAI 兼容一般含 /v1）',
  'connect.model': '模型名称（如 deepseek-chat）',
  'connect.setDefault': '设为默认模型？',
  'connect.defaultYes': '是',
  'connect.defaultNo': '否',
  'connect.saved': '已保存供应商「${label}」，模型 ${model}（key: ${key}）',
  'connect.cancel': '已取消，未保存任何配置。',
  'connect.invalid': '输入无效：${msg}',
  'connect.type.openai-compatible': 'OpenAI 兼容（/chat/completions）',
  'connect.type.openai-responses': 'OpenAI Responses API',
  'connect.type.anthropic': 'Anthropic（Claude）',
  'connect.type.gemini': 'Google Gemini',
  'connect.mcpNotImpl': 'MCP 交互向导暂未实现，可在桌面端「设置 → MCP」添加，或编辑 ~/.voked/config.json 的 mcpServers。',
...
const en: Dict = {
  ...
  'cli.noModelHint': 'No model configured yet — type /connect provider to add a provider, then start chatting.',
  ...
  'cmd.connect.desc': 'Interactively add a model provider',
  'connect.usage': 'Usage: /connect provider',
  'connect.title': 'Connect wizard',
  'connect.providerTitle': 'Add model provider',
  'connect.step': 'Step ${i}/${n}',
  'connect.chooseType': 'Select protocol type',
  'connect.label': 'Provider label (for display, e.g. DeepSeek)',
  'connect.apiKey': 'API Key',
  'connect.baseURL': 'Base URL (optional; OpenAI-compatible usually ends with /v1)',
  'connect.model': 'Model name (e.g. deepseek-chat)',
  'connect.setDefault': 'Set as default model?',
  'connect.defaultYes': 'Yes',
  'connect.defaultNo': 'No',
  'connect.saved': 'Saved provider "${label}", model ${model} (key: ${key})',
  'connect.cancel': 'Cancelled, nothing saved.',
  'connect.invalid': 'Invalid input: ${msg}',
  'connect.type.openai-compatible': 'OpenAI compatible (/chat/completions)',
  'connect.type.openai-responses': 'OpenAI Responses API',
  'connect.type.anthropic': 'Anthropic (Claude)',
  'connect.type.gemini': 'Google Gemini',
  'connect.mcpNotImpl': 'Interactive MCP wizard is not implemented yet. Add it in desktop "Settings → MCP", or edit mcpServers in ~/.voked/config.json.',
